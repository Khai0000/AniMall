  ...
  if (hasLocalStorageAuth) {
    try {
      const responseAuth = await axios.get(
        `${ENDPOINT_CHECK_AUTH}?email=${localStorageAuth.data.email}`,
        {
...
// send post to login
export const serverLoginUser = async (formLogin) => {
  let serverResponse = {
    data: {
      status: 0,
    },
  ...
  try {
    const responseLogin = await axios.post(ENDPOINT_LOGIN, formData);
    if (responseLogin.data.status === 1) {
      ...
      responseLogin.data.status === 2 ||
      responseLogin.data.status === 3
    ) {
      serverResponse = {
        data: {
          status: responseLogin.data.status,
