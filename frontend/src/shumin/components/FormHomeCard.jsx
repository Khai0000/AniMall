...
const FormHomeCard = ({ form, onPostDelete, update }) => {
    ...
    try {
      ...
      const response = await axios.post(
        `http://localhost:4000/api/reminders//adoptionemail`, { 
          email: form.email ,message :"Thank you for your kindness.We regret to inform you that your adoption application is has not been successful."
        });
  ...
  const whenApprove = async (e) => {
    ...
    try {
      ...
      const response = await axios.post(
        `http://localhost:4000/api/reminders//adoptionemail`, { 
          email: form.email ,message :"Thank you for your kindness.We are happy to inform you that your adoption application is has been successful.Kindly come to our shop for further action as soon as possible."
        });
      if (response.status === 200 && response.data.message === 'Email sent successfully') {
    ...
    try {
      ...
      if (deletePostResponse.status === 200) {
        ...
        console.error("Unexpected response status:", deletePostResponse.status);
        setSentMessage('An error1 occurred while approving the form. Please try again later.');
        setMessage(true);
      }
    } catch (error) {
      console.error("Error deleting form:", error);
      setSentMessage('An error2 occurred while approving the form. Please try again later.');
      setMessage(true);
    }
  };
  ...
  return (
    ...
    >
      ...
      {showAdoptFormCheck && <AdoptFormCheck showAdoptFormCheck={showAdoptFormCheck} setShowAdoptFormCheck={setShowAdoptFormCheck} whenApprove={whenApprove} whenReject={whenReject} form={form} />}
      <AdoptFormUpdate show={showUpdateForm} onClose={() => { setShowUpdateForm(false); console.log("Clicked"); console.log(showUpdateForm); }} form={form} />
      <SuccessMessage show={message} onClose={() => { setMessage(false); setShowUpdateForm(false); onPostDelete(form._id); }} message={sentMessage}></SuccessMessage>
    </div>
  );
}
